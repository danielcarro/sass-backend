...
@section('content')
    <div class="container mx-auto px-4 py-8 max-w-3xl">
        <h1 class="text-3xl font-bold mb-6 text-gray-800 dark:text-gray-100">Editar Tenant</h1>
        ...
        <form action="{{ route('tenants.update', $tenant) }}" method="POST"
            ...
            <div class="mb-6">
               ...
               dark:focus:ring-blue-400"
                    required>
                    <option value="" disabled>-- Selecione o usuário principal --</option>
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}"
                            {{ old('user_id', optional($tenant->users->first())->id) == $user->id ? 'selected' : '' }}>
